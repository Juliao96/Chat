...
namespace Chat.Common
{
    ...
    // Запускаем в отдельном thread и читаем, что поступает во входной stream.
    public class InputHandler
    {
        // Eventos disparados em decorrência da chegada de comandos.
        // События, вызванные поступившими командами.
        ...
        // Указывает, следует ли остановить цикл чтения.
        bool stop;
        ...
        public void Start()
        {
            ...
            // Поток имеет тип background, потому что, если приложение завершается, поток завершается.
            t.IsBackground = true;
        ...
        // Каждый из приведенных ниже методов обрабатывает определенную команду. Цель состоит в том, чтобы вызвать соответствующее событие, например это
        // кто регистрируется на событие знает, какая команда пришла.
        // Тот, кто получает событие, может завершить цикл чтения потока, установив свойство StopInputHandler
        ...
        private void HandleGetMembersResponse(ChatCommand command)
        {
            if (GetMembersResponse != null)
            {
                ...
                // Список членов представляет собой строку, разделенную ","
